...
def sales_wb(mpid, key_type):
    ...
    def limit_fix_function(datefrom):
        request_body = InitialData.wb_sales_url + '?dateFrom=' + str(datefrom) + '&flag=0'
    ...
    if isinstance(df_full, int):
        ...
        return df_full
    print(f'initial size: {df_full.size}')
    df_full.drop_duplicates()
    print(f'off-duplicates size: {df_full.size}')

    type_sql = 'append'
    query = None
    # sh_flag - нужно ли отправлять данные в google spreadsheets
    sh_flag = False
    insertion_object = InsertionMeta(df_full, 'SalesWb', 'sales_wb_new', mpid, query, type_sql, sh_flag)
