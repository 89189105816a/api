...
@logger_decorate
def orders_daily_wb(mpid, interval, key_type='wildberries-get'):
    print('---orders_daily_wb---')
    ...
    for element in time_delay_list_corrected:
        request_body = InitialData.wb_orders_url + '?dateFrom=' + str(element) + '&flag=1'
        # print(request_body)
        # print(time_delay_list_corrected[-1])
        ...
        sh_flag = False
        insertion_object = InsertionMeta(df, 'Orders_daily_wb', 'orders_wb_new', mpid, query, type_sql, sh_flag)
